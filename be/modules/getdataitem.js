...
function getPermissionType(comment) {
	...
	if(hasPermission) {
		...
		if(isSpecificPermission) {
			...
			const similaAll = ['all', 'many', 'much', 'some'].reduce((acc, item) => {
				return [...acc, ...getMostSimilarWords(item)]
			}, [])
			const hasAllPermission = findKeydsInText(similaAll, comment).length

			if(hasAllPermission) hasPermissionType = 'all'
		}
	}
...
function getDataCollectionType(comment) {
	...
	if(hasSimila) {
		...
		if(isSpecific) {
			...
			if(hasAllPermission) hasType = 'all'
		}
	}
...
function getSharingType(comment) {
	...
	let simila = getMostSimilarWords('sharing')
	const hasSimila = findKeydsInText(simila, comment).length
